...
3. ✅ **缩放级别优化**：优化初始缩放为3级（显示3x），最大缩放为17级（显示15x）
...
6. ✅ 调整最大缩放级别
   - 从 22 级（20x）降低到 17 级（15x）
   - 更新缩放映射函数：3-17 映射到 1-15x
   - 统一所有地图源的最大缩放为 17 级
...
- ⏳ **待浏览器实测**：完成自动降级触发验证（含配额 403 日志）与 CORS 检查后方可关闭。
