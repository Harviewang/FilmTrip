...
**目标**：
- 上传/编辑照片时，通过地图选点自动回填五级中文地址与 `latitude/longitude`
- 处理已存在数据的回填逻辑，避免中文/英文混用造成地图显示错误
- 在标准预览模式点击「省/市/区」字段，弹出带中文标注的小地图定位

**计划拆解**：
1. 🔁 **地址解析服务（M-4）**：提供坐标 ↔ 省/市/区/街道（五级）解析；字段沿用 `country/province/city/district/township` + `latitude/longitude`，主要使用 MapTiler API，文档中预留 Geomaply 作为后续备选。
2. 📥 **上传/编辑联动（M-5）**：上传/编辑表单通过地图选点触发解析并回填字段；流程为“选点 → 调用后台解析 → 用户确认 → 写库”，不再提供纯文本地址输入。
3. 🗺️ **预览小地图（M-6）**：MapLibre 按需加载，点击省/市/区字段弹出迷你地图，默认 `zoom = 12`，仅在预览中展示中文标签与标记（列表视图不加载、无需跳转大地图）。
4. 🌏 **翻译策略**：优先使用 MapTiler 返回的中文；若字段为英文且包含 `country_code`，通过内置映射表翻译，必要时才调用第三方翻译 API。
5. 📘 **最佳实践**：详见 `docs/knowledge-base/best-practices/地图中文地点增强最佳实践-2025-10-28.md`。
