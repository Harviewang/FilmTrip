...
**修复**：
- 添加`?limit=100`参数（后端API限制最大100）
- 如果照片数量超过100，建议后续改为分页请求或使用专门的地图接口
...
4. **数据获取**：使用`?limit=100`确保地图显示更多可见照片（后端API限制最大100），而不是默认的20张
   - **注意**：如果照片数量超过100，建议后续改为分页请求或使用专门的地图接口
