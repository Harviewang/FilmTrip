...
1. **地图标记点击无反应**（修复于 `5e9b987`）
   ...
   - **修复**：移除图片路径存在性检查，恢复直接调用 `setSelectedPhoto(photo)`，让 PhotoPreview 组件自己处理图片路径不可用的情况
...
3. 如果后续需要在点击时过滤无法查看的照片，可以考虑：
   - 点击时补一次 `/api/photos/:id` 详情请求，拿到真实可用的图片路径再放行
   - 或者检查 `photo.filename` 等更稳妥的标识作为判断依据
...
4. **图片路径检查**：只有当至少有一个图片路径字段存在时才打开预览
