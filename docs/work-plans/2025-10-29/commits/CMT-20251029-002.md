...
## Git 提交记录
- `31dfa5e`: fix: 修复API返回的胶卷和相机数据不显示问题
- `35a0dea`: fix: 后台管理页面正确显示胶卷名称
- `d504609`: fix: 前端预览页面正确显示胶卷名称
- `7852d30`: refactor: 调整胶卷显示优先级 - 优先显示胶卷规格而非实例名称
- `be40063`: fix: Gallery组件使用后端API的film和camera字段
- `f63e0f3`: fix: 修复taken_date字段在API返回中丢失的问题
- `2177f71`: feat: 沉浸预览模式背景改为黑色
- `982f5e6`: docs: 更新CMT-20251029-002文档
- `41c02bc`: fix: 修复日本城市翻译逻辑并更新文档说明
...
### 审计反馈与修复
1. **日本城市翻译问题** ✅ 已修复
   - 问题：`geocode-translations.js` 未根据 `level` 参数调用城市翻译函数
   - 修复：添加 `level === 'city'` 判断，正确调用 `getJapanCityTranslation` 等函数
   
2. **PhotoPreview信息栏问题** ✅ 已澄清
   - 问题：审计发现EXIF信息被移除，担心违反"元数据增强"需求
   - 澄清：胶片摄影不记录EXIF信息（光圈/快门/ISO），底部信息栏显示6个核心字段符合胶片摄影特性
   - 文档更新：同步更新所有相关文档，明确胶片摄影特性
