...
- **数据库层**
  ...
  - `storage_variant` 取值建议：`RAW`（原始文件，仅管理员可见、不加水印）、`WEB`（标准预览，默认加水印，面向普通访问）、`THUMB`（缩略图，用于列表与懒加载，不加水印以保证清晰度/性能）、`SHARE`（社交分享专用，加水印并叠加分享所需的标识/文案）；必要时可扩展，如需新增必须同步更新枚举与文档。
  - `short_code` 规则：Base62 编码、长度 6~8 字符，冲突重试次数 ≥3；对外展示统一前缀 `https://filmtrip.cn/s/{short_code}`。
