...
## 审计状态
✅ **Codex 审计通过（2025-10-29）** → ⚠️ **发现语法错误，已修复**

**审计结论**：
- `frontend/src/pages/Map/MapLibre.jsx#L169` 起采用 while 循环分页请求 `/api/photos?page={N}&limit=100`，合并所有可见照片后再映射，确保地图不再只显示前 20 张
- 标记点击事件两处 (`MapLibre.jsx#L206`、`#L434/#L476`) 均直接调用 `setSelectedPhoto(photo)`，预览弹窗恢复正常，权限过滤仍通过图片路径字段保障
- `docs/work-plans/2025-10-29/commits/CMT-20251029-003.md` 已同步新增分页方案、原因与后续聚合建议，文档示例和现行代码保持一致
- 当前实现与文档状态一致，可继续下一步流程

**后续发现的问题**：
- ⚠️ **语法错误**：`MapLibre.jsx` 第246行多了一个闭合花括号 `}`，导致 try-catch 语法错误，页面无法打开
- ✅ **已修复**：删除多余的花括号，代码结构恢复正常，构建通过
- **原因分析**：之前修改时引入了多余的花括号，导致代码无法编译
- **修复提交**：见 Git 提交记录
...
### 6. 修复地图数据获取限制
**问题**：地图只展示前20张照片（后端API默认limit=20）
- MapLibre组件使用默认`/api/photos`请求，只返回20张照片
- 导致地图上只显示部分照片的标记

**用户需求**：`/map`不应该分页，应该显示全部可见照片，后续可以考虑聚合

**修复**：
- 实现分页循环请求，自动获取所有可见照片
- 每页请求100张（后端API限制最大100）
- 循环直到获取所有数据，然后统一处理
- 后续优化：考虑对相同坐标的照片进行聚合（合并标记显示）

**实现逻辑**：
1. 使用while循环，从page=1开始
2. 每次请求`/api/photos?page=X&limit=100`
3. 检查`result.pagination.pages`判断是否还有更多数据
4. 过滤掉没有图片路径的照片（权限过滤）
5. 合并所有分页数据后统一映射

**Commits**:
- `cd3de04`: 初步修复limit问题（误设为1000，后端不支持）
- `21d248f`: 调整为后端支持的最大值100
- `a4de4d3`: 实现分页循环请求，获取全部可见照片
...
4. **后续聚合优化**：考虑对相同坐标的照片进行聚合显示，提升大量标记时的性能
   - 相同坐标的多张照片可以合并成一个聚合标记
   - 点击聚合标记时可以展开显示该位置的所有照片
   - 可以参考地图库的聚合插件实现
...
cd3de04 fix: 修复地图数据获取限制和文档代码片段（初步修复limit=1000）
21d248f fix: 调整地图数据获取limit为后端支持的最大值100
a4de4d3 feat: 地图分页获取全部可见照片，不再只显示前100张
```
...
4. **数据获取**：实现分页循环请求，自动获取全部可见照片
   - 使用while循环，从page=1开始，每次请求100张（后端API最大限制）
   - 检查`result.pagination.pages`判断是否还有更多数据
   - 合并所有分页数据后统一处理
   - **后续优化**：考虑对相同坐标的照片进行聚合（合并标记显示）
